...
def crow_first(keyword,n):
    # 构造每一页的url变化
    url = 'http://search.jd.com/Search?keyword='+str(keyword)+'&enc=utf-8&page=%d'+str(2 * n - 1)
    ...
    # 将抓取的结果保存到本地CSV文件中
    with open('JD.csv', 'a', newline='', encoding='utf-8')as f:
        write = csv.writer(f)
...
def Save_Database():
    ...
    # 4.修改数据
    reader = csv.reader(open(r'JD.csv', 'r',encoding='utf-8'))
    for item in reader:
